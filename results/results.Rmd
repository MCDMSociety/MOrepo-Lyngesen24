...
## Table to summarize computational results

Load data from minimum generator sets study:
```{r}
dat_MGS <- read_csv(here::here("results/results_minimum_generator_sets.csv"))
...
dat_compare_instances <- dat_MGS %>% 
   group_by(max_size,m,spAveCard,p) %>% 
   summarise(count = n(),MGS_size = mean(MGS_size), rel_size = mean(MGS_size/max_size), Yn_size = mean(Yn_size), sol_time = mean(running_time)) %>% group_by(max_size) %>%
   ungroup()

# show table
DT::datatable(dat_compare_instances %>% select(-max_size) %>% mutate(rel_size = percent(rel_size,accuracy = decimals), MGS_size = round(MGS_size,1), Yn_size = round(Yn_size, digits = 1), sol_time = round(sol_time, 2)))

```

Load data from reduced generator sets:
